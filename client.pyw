...
class Circle():
	...
	def data(self):
		#Max 16 bytes can be sent via socket. Data is a as follows
...
while retry:
	try:
		...
		time.sleep(4)
		continue
	#print("connection to the server has been made")
	retry = not retry
	"""
	try:
		threading.Thread(target=send_receive, args=client)
	except:
		print("Connection has been closed by remote host. Please restart the client")
	"""
...
while live:
	for event in pygame.event.get():
		if event.type == pygame.QUIT:	#essential
			client.send("quit".encode("utf-8"))
			live = False
			time.sleep(.5)
			pygame.quit()
